...
> R: La función carrito_init inicializa la cantidad de elementos del carrito en 0. Si no la usamos, entonces tendríamos basura dentro del carrito al hacer el test.
...
> R: Falla. Esperaba 700 y obtuve 350.
...
> R: El bug está en la línea 24 del archivo `src/carrito.c`. El código dice `total += c->items[i].precio;`

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: Debería de multiplicar el precio por la cantidad de ese producto en el carrito.
...
> R:El test falla, devuelve 1 cuando se espera 0.
...
> R: Sí, las líneas 29 y 30 no se ejecutaron porque no se probó la función `carrito_descuento`.
...
> R: Un test unitario prueba una función específica de manera aislada, mientras que un test de integración verifica cómo varias funciones trabajan juntas. El bug de `carrito_total` fue detectado primero por un test unitario.
...
> R: En C, los arrays no tienen verificación de límites, por lo que escribir más allá de su tamaño puede sobrescribir memoria adyacente, causando comportamientos impredecibles o vulnerabilidades de seguridad. En Python o Java, los arrays y listas tienen verificación de límites, y lanzarían una excepción si se intenta acceder fuera del rango permitido.
...
> R: Escribir tests permite verificar el comportamiento del código en diferentes escenarios y detectar errores que podrían no ser evidentes al leer el código. Además, los tests proporcionan una forma automatizada de validar cambios futuros, asegurando que las correcciones no rompan funcionalidades existentes.
...
> R: El primer test no detectó el bug porque solo probaba un caso donde la cantidad era 1, y el cálculo del total coincidía con el precio unitario. El bug afectaba el cálculo cuando la cantidad era mayor a 1, lo que se verificó en el segundo test.
